...
${suffix}`;
  ...
  // src/extension/subscriptions-app.ts
  var currentUserId = "";
  ...
  function checkSubscription() {
    chrome.storage.local.get(["infolders_premium"], (result) => {
      if (isPremiumFromStorage(result.infolders_premium)) {
        showStatus("\u2705 Sei gi\xE0 Premium! Goditi tutte le funzionalit\xE0.", "success");
        disableButtons();
  ...
  async function syncPremiumFromSupabase() {
    try {
  ...
  function handlePlanSelect(plan) {
    if (plan === "free") {
      showStatus("Sei gi\xE0 sul piano Free.", "info");
      return;
    }
    if (!currentUserId) {
    ...
    createCheckoutSession(plan, currentUserId, currentUserEmail).then((url) => {
      window.open(url, "_blank");
      showStatus("Pagamento avviato. Completa il checkout nella nuova finestra.", "info");
    }).catch((err) => {
      showStatus("Errore: " + (err.message || "Riprova pi\xF9 tardi."), "info");
    });
  }
