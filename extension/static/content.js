"use strict";
(() => {
  // src/extension/lib/lucide-icons.ts
  var icons = {
    ...
    edit3: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>',
    ...
    "folder-plus": '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 10v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.93a2 2 0 0 1-1.66-.9l-.82-1.2A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v5Z"/><line x1="12" y1="10" x2="12" y2="16"/><line x1="9" y1="13" x2="15" y2="13"/></svg>',
    "folder-cog": '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.5 20H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h3.93a2 2 0 0 1 1.66.9l.82 1.2a2 2 0 0 0 1.66.9H18a2 2 0 0 1 2 2v3"/><circle cx="18" cy="18" r="3"/><path d="M18 14v1"/><path d="M18 21v1"/><path d="M14 18h1"/><path d="M21 18h1"/></svg>',
    ...
    "folder-symlink": '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m2 10 3-3 3 3"/><path d="M22 10v9a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-9a2 2 0 0 1 2-2h3"/><path d="M2 7h10a4 4 0 0 1 4 4v3"/></svg>',
    ...
    palette: '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 14.7255 3.09032 17.1962 4.85857 19C5.03345 19.1749 5.09902 19.4294 5.0232 19.6644C4.85303 20.1919 4.75971 20.7483 4.75971 21.3241C4.75971 21.6974 5.06231 22 5.43564 22H12Z"/><circle cx="7.5" cy="10.5" r="1.5"/><circle cx="11.5" cy="7.5" r="1.5"/><circle cx="16.5" cy="9.5" r="1.5"/><circle cx="15.5" cy="14.5" r="1.5"/></svg>'
  };
  var LucideIcons = {
    get(name, size = 16) {
    ...
    element(name, size = 16, className = "") {
      const svgStr = this.get(name, size);
      if (!svgStr) return document.createElement("span");
  ...
  var lucide_icons_default = LucideIcons;

  // src/extension/lib/platform-config.ts
  ...
  function filterFolderTreeByQuery(nodes, query) {
    const q = (query || "").trim().toLowerCase();
    if (!q) return nodes || [];
    function walk(folders2) {
      const out = [];
  ...
  var LANGUAGES = {
    it: {
      ...
      strings: {
        ...
        "tools.clear.confirm": "Sei sicuro di voler cancellare tutti i dati? Questa azione \xE8 irreversibile.",
        ...
        "premium.free.current.info": "Sei gi\xE0 sul piano Free.",
        ...
        "premium.pro.badge": "PI\xD9 SCELTO",
...
${suffix}`;
  ...
  // src/extension/lib/prompts.ts
  var STORAGE_KEY2 = "infolders_prompts";
  ...
  function addPrompt(title, content, category) {
    const prompts = loadPrompts();
    const prompt = { id: Date.now(), title, content, category, createdAt: (/* @__PURE__ */ new Date()).toISOString() };
  ...
  function filterPrompts(query, category) {
  ...
  // src/extension/lib/instruction-profiles.ts
  var STORAGE_KEY3 = "infolders_profiles";
  var ACTIVE_KEY = "infolders_active_profile";
  ...
  function saveProfiles(list) {
    localStorage.setItem(STORAGE_KEY3, JSON.stringify(list));
  }
  function addProfile(name, instructions, description, color) {
    const profiles = loadProfiles();
    const profile = { id: Date.now(), name, instructions, description, color };
  ...
  function showToast(message, type = "info", duration = 3e3) {
    ...
    switch (type) {
      case "success":
        icon = lucide_icons_default.get("check-circle", 16);
        bgColor = `${theme.primary}25`;
        borderColor = theme.primary;
        break;
      case "error":
        icon = lucide_icons_default.get("alert-circle", 16);
        bgColor = "rgba(239, 68, 68, 0.15)";
        borderColor = "#ef4444";
        break;
      case "warning":
        icon = lucide_icons_default.get("alert-triangle", 16);
        bgColor = "rgba(245, 158, 11, 0.15)";
        borderColor = "#f59e0b";
        break;
      default:
        icon = lucide_icons_default.get("info", 16);
  ...
  async function startStripeCheckout(plan) {
    ...
    try {
      showToast("Reindirizzamento a Stripe...", "info", 3e3);
  ...
  function updateInPageButtonsPosition() {
  ...
  function createRoundButton(id, iconName, title, theme) {
    ...
    btn.appendChild(lucide_icons_default.element(iconName, 20));
  ...
  function openSidebar() {
    ...
    allTabs.forEach((tab) => {
      ...
      btn.innerHTML = `<span style="display:flex;align-items:center;justify-content:center;flex-shrink:0;">${lucide_icons_default.get(tab.icon, 16)}</span><span class="tab-label" style="opacity:0;max-width:0px;overflow:hidden;white-space:nowrap;transition:all 0.25s cubic-bezier(0.25, 0.8, 0.25, 1);font-size:11px;font-weight:700;line-height:1;margin-left:0px;">${tabLabels[tab.id]}</span>`;
      btn.addEventListener("mouseenter", () => {
    ...
    closeBtn.innerHTML = lucide_icons_default.get("x", 18);
  ...
  function updateTabStyles(activeTabId) {
    const theme = currentTheme;
    document.querySelectorAll("#infolders-sidebar button[data-tab]").forEach((btn) => {
    ...
    `;
      ...
      <div style="padding:8px 0 16px;">
        <div style="display:flex;align-items:center;gap:14px;padding:16px;background:${theme.surface};border:1px solid ${theme.border};border-radius:16px;margin-bottom:16px;">
          <div style="width:52px;height:52px;border-radius:14px;background:${theme.appAccent}18;border:2px solid ${theme.appAccent}33;display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;">${currentUser.picture ? `<img src="${escapeHtml(currentUser.picture)}" alt="" style="width:100%;height:100%;object-fit:cover;" />` : `<span style="color:${theme.appAccent};">${lucide_icons_default.get("user", 24)}</span>`}</div>
  ...
  function renderToolsTab(container) {
    const theme = currentTheme;
    const mkBtn = (id, icon, label, sub, danger = false) => `
    <button id="${id}" style="background:${danger ? "rgba(239,68,68,0.06)" : theme.surface};border:1px solid ${danger ? "rgba(239,68,68,0.2)" : theme.border};color:${danger ? "#f87171" : theme.textPrimary};padding:14px 16px;border-radius:14px;cursor:pointer;text-align:left;display:flex;align-items:center;gap:14px;font-size:13px;font-weight:500;transition:all 0.2s ease;width:100%;">
      <span style="width:36px;height:36px;border-radius:10px;flex-shrink:0;background:${danger ? "rgba(239,68,68,0.12)" : `${theme.appAccent}15`};display:flex;align-items:center;justify-content:center;color:${danger ? "#f87171" : theme.appAccent};">${lucide_icons_default.get(icon, 18)}</span>
  ...
  function renderFoldersTab(container) {
    const theme = currentTheme;
    container.innerHTML = `
    <div style="padding:0 0 8px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        ...
        <button id="sidebar-add-folder" style="background:${theme.appAccent};border:none;color:white;padding:6px 14px;border-radius:8px;cursor:pointer;display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;letter-spacing:0.2px;transition:all 0.2s ease;box-shadow:0 2px 10px ${theme.appAccent}33;">${lucide_icons_default.get("plus", 12)} Nuova</button>
      </div>
      <div style="position:relative;margin-bottom:12px;">
        <span style="position:absolute;left:11px;top:50%;transform:translateY(-50%);color:${theme.textSecondary};pointer-events:none;display:flex;">${lucide_icons_default.get("search", 13)}</span>
  ...
  `;
    ...
    const searchEl = container.querySelector("#sidebar-folder-search");
    searchEl.addEventListener("input", () => {
      folderSearchQuery = searchEl.value;
      renderFoldersList(container);
    });
    searchEl.addEventListener("focus", function() {
      this.style.borderColor = theme.appAccent;
    });
  ...
  function renderFoldersList(container) {
    const list = container.querySelector("#sidebar-folders-list");
    if (!list) return;
    list.innerHTML = "";
  ...
  function getFolderIconElement(folder, isExpanded, color) {
    ...
    return lucide_icons_default.element(iconName, 15);
  }
  function createFolderElement(folder, parent, depth) {
    const theme = currentTheme;
    ...
    expandBtn.addEventListener("click", (e) => {
      ...
      const sub = folderDiv.querySelector(".subfolders");
    ...
    addSubBtn.innerHTML = lucide_icons_default.get("plus", 12);
    ...
    addSubBtn.addEventListener("mouseenter", () => {
      addSubBtn.style.background = theme.surfaceHover;
      addSubBtn.style.color = effectiveColor;
    });
    addSubBtn.addEventListener("mouseleave", () => {
      addSubBtn.style.background = "transparent";
      addSubBtn.style.color = theme.textSecondary;
    });
    addSubBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      addSubfolderFromSidebar(folder.id);
    });
    const deleteBtn = document.createElement("button");
    deleteBtn.title = "Elimina cartella";
    deleteBtn.innerHTML = lucide_icons_default.get("trash2", 12);
    ...
    deleteBtn.addEventListener("mouseleave", () => {
      ...
      deleteBtn.style.color = theme.textSecondary;
    });
    deleteBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      deleteFolderFromSidebar(folder.id);
    });
  ...
  function addProjectFromSidebar(parentId) {
    const allowed = getFolderColors();
  ...
  function addChatToFolderFromContextMenu(folder) {
    showAddChatPickerDialog(folder);
  }
  function showAddChatPickerDialog(folder) {
    const theme = currentTheme;
    const config = getPlatformConfig();
    ...
    const alreadyAdded = new Set((folder.chats || []).map((c) => c.url));
    ...
    const closeBtn = document.createElement("button");
    closeBtn.innerHTML = lucide_icons_default.get("x", 14);
    Object.assign(closeBtn.style, {
      background: "transparent",
      border: "none",
      color: theme.textSecondary,
    ...
    searchIcon.innerHTML = lucide_icons_default.get("search", 13);
    ...
    if (noChatsFromPage || allChats.length < 5) {
      ...
      banner.innerHTML = `
      <span style="flex-shrink:0;margin-top:1px;">${lucide_icons_default.get("triangle-alert", 13)}</span>
      <div>
    ...
    `;
      banner.querySelector("#infolders-import-all-link").addEventListener("click", () => {
        showToast("Scorri la sidebar del chatbot per caricare pi\xF9 chat, poi riapri questo dialog.", "info");
      });
  ...
  `;
    ...
    clearBtn.addEventListener("mouseenter", () => {
      clearBtn.style.borderColor = theme.border;
      clearBtn.style.color = "#fff";
    });
    clearBtn.addEventListener("mouseleave", () => {
      clearBtn.style.borderColor = `${theme.border}55`;
      clearBtn.style.color = theme.textSecondary;
    });
    ...
    addBtn.addEventListener("click", () => {
      ...
      chrome.storage.local.get(["infolders_premium"], (r) => {
        ...
        for (const c of toAdd) {
          if (!folder.chats.some((existing2) => existing2.url === c.url)) {
  ...
  function getValidMoveTargets(nodes, excludeId, prefix = "") {
    let out = [];
  ...
  function findAndRemoveFolder(nodes, id) {
  ...
  function showMoveFolderDialog(folder) {
    const theme = currentTheme;
    const overlay = document.createElement("div");
    Object.assign(overlay.style, {
      position: "fixed",
    ...
    const optionsHtml = validTargets.map((t2) => `<option value="${t2.id}">${escapeHtml(t2.name)}</option>`).join("");
    modal.innerHTML = `
  ...
  `;
    ...
    modal.querySelector("#move-save").addEventListener("click", () => {
      const target = modal.querySelector("#move-target-select").value;
  ...
  function showEditLogoDialog(folder) {
    const theme = currentTheme;
    const overlay = document.createElement("div");
    Object.assign(overlay.style, {
      position: "fixed",
    ...
    const presets = ["\u{1F4C1}", "\u{1F4BC}", "\u{1F393}", "\u{1F4BB}", "\u{1F916}", "\u{1F4DD}", "\u{1F31F}", "\u{1F525}", "\u{1F4A1}", "\u{1F4CA}", "\u2764\uFE0F", "\u2708\uFE0F"];
    const presetsHtml = presets.map(
      (p) => `<button class="logo-preset-btn" data-value="${p}" style="background:rgba(255,255,255,0.05);border:1px solid ${theme.border};color:white;width:36px;height:36px;border-radius:8px;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.15s ease;">${p}</button>`
    ).join("");
    modal.innerHTML = `
    ...
    <div style="margin-bottom:16px;">
      ...
      <input id="custom-logo-input" value="${escapeHtml(folder.icon || "")}" placeholder="es: \u{1F680}" style="width:100%;box-sizing:border-box;padding:10px 12px;border-radius:8px;border:1px solid ${theme.border};background:${theme.glassBg};color:#fff;font-size:13px;outline:none;">
    </div>
    <div style="display:flex;gap:8px;justify-content:flex-end;">
  ...
  `;
    overlay.appendChild(modal);
    document.body.appendChild(overlay);
    const customInput = modal.querySelector("#custom-logo-input");
    modal.querySelectorAll(".logo-preset-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        customInput.value = btn.dataset.value;
        customInput.focus();
      });
    });
  ...
  function showChangeColorDialog(folder) {
    const theme = currentTheme;
    const overlay = document.createElement("div");
    Object.assign(overlay.style, {
      position: "fixed",
    ...
    const colorSwatches = availableColors.map(
      (c) => `<button class="change-color-swatch" data-color="${c}" style="width:32px;height:32px;border-radius:8px;background:${c};border:${c === selectedColor ? "3px solid white" : "2px solid transparent"};cursor:pointer;transition:all 0.15s ease;"></button>`
    ).join("");
    modal.innerHTML = `
    ...
    overlay.querySelectorAll(".change-color-swatch").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        overlay.querySelectorAll(".change-color-swatch").forEach((b) => {
          b.style.border = b.dataset.color === selectedColor ? "3px solid white" : "2px solid transparent";
        });
      });
    });
  ...
  function showFolderContextMenu(e, folder) {
    ...
    const createMenuItem = (icon, label, onClick, hasInfo = false) => {
      ...
      iconSpan.innerHTML = lucide_icons_default.get(icon, 14);
  ...
  function renderBookmarksTab(container) {
    const theme = currentTheme;
    container.innerHTML = `
    <div style="padding:0 0 8px;">
  ...
  function renderBookmarksList(container) {
    ...
    if (bookmarks.length === 0) {
      ...
      <div style="text-align:center;padding:40px 16px;">
        <span style="color:${theme.appAccent};display:flex;justify-content:center;margin-bottom:12px;opacity:0.5;">${lucide_icons_default.get("bookmark", 28)}</span>
    ...
    bookmarks.forEach((bookmark, idx) => {
      ...
      Object.assign(item.style, {
        ...
        border: `1px solid ${theme.border}`,
        borderLeft: `3px solid ${theme.appAccent}`,
        borderRadius: "12px",
        display: "flex",
        flexDirection: "column",
        gap: "8px",
      ...
      item.addEventListener("mouseenter", () => {
        item.style.background = theme.surfaceHover;
        item.style.borderColor = `${theme.appAccent}44`;
        item.style.borderLeftColor = theme.appAccent;
      });
      item.addEventListener("mouseleave", () => {
      ...
      <span style="display:flex;align-items:center;gap:4px;">
        <span style="color:${theme.appAccent};opacity:0.7;">${lucide_icons_default.get("message-circle", 10)}</span>
        ${escapeHtml(bookmark.platform || "Generico")}
      </span>
      <button class="remove-bookmark" data-idx="${idx}" style="background:none;border:none;color:${theme.textSecondary};cursor:pointer;display:flex;align-items:center;padding:4px 5px;border-radius:6px;transition:all 0.15s ease;">${lucide_icons_default.get("x", 13)}</button>
    `;
  ...
  function showFolderDialog(title, initialName, initialColor, confirmLabel, onConfirm) {
    const theme = currentTheme;
    const overlay = document.createElement("div");
    Object.assign(overlay.style, {
      position: "fixed",
    ...
    const colorSwatches = availableColors.map(
      (c) => `<button class="folder-color-swatch" data-color="${c}" style="width:28px;height:28px;border-radius:8px;background:${c};border:${c === selectedColor ? "3px solid white" : "2px solid transparent"};cursor:pointer;transition:all 0.15s ease;"></button>`
    ).join("");
    modal.innerHTML = `
  ...
  `;
    ...
    requestAnimationFrame(() => modal.querySelector("#folder-name-input").focus());
  }
  ...
  function addSubfolderFromSidebar(parentId) {
    const allowed = getFolderColors();
  ...
  function findFolderById(arr, id) {
    for (const f of arr) {
      if (f.id === id) return f;
      if (f.subfolders) {
  ...
  function renderPromptsTab(container) {
    ...
    <div style="padding:0 0 8px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        ...
        <button id="add-prompt-btn" style="background:${theme.appAccent};border:none;color:white;padding:6px 14px;border-radius:8px;cursor:pointer;display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;letter-spacing:0.2px;transition:all 0.2s ease;box-shadow:0 2px 10px ${theme.appAccent}33;">${lucide_icons_default.get("plus", 12)} Nuovo</button>
      </div>
      <div style="position:relative;margin-bottom:12px;">
        <span style="position:absolute;left:11px;top:50%;transform:translateY(-50%);color:${theme.textSecondary};pointer-events:none;display:flex;">${lucide_icons_default.get("search", 13)}</span>
  ...
  `;
    const catContainer = container.querySelector("#prompt-categories");
    if (catContainer) {
    ...
    const searchEl = container.querySelector("#prompt-search");
  ...
  function renderPromptList(container, category) {
    const theme = currentTheme;
    const list = container.querySelector("#prompts-list");
    if (!list) return;
    const q = (container.querySelector("#prompt-search")?.value || "").trim();
    const prompts = filterPrompts(q, category || void 0);
    list.innerHTML = "";
    if (prompts.length === 0) {
      list.innerHTML = `<div style="text-align:center;padding:40px 16px;"><span style="color:${theme.appAccent};display:flex;justify-content:center;margin-bottom:12px;opacity:0.5;">${lucide_icons_default.get("file-text", 28)}</span><p style="color:${theme.textSecondary};font-size:12px;line-height:1.6;margin:0;">${q ? `Nessun prompt per "${escapeHtml(q)}"` : "Nessun prompt. Clicca <strong>Nuovo</strong> per aggiungerne uno."}</p></div>`;
      return;
    }
    prompts.forEach((p) => {
      ...
      card.addEventListener("mouseenter", () => {
        ...
        card.style.borderColor = `${theme.appAccent}33`;
      });
      ...
      <div style="display:flex;gap:6px;">
        <button class="prompt-copy" data-id="${p.id}" style="flex:1;background:${theme.appAccent};border:none;color:white;padding:7px;border-radius:8px;cursor:pointer;font-size:11px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:5px;transition:all 0.15s ease;">${lucide_icons_default.get("clipboard", 12)} Copia</button>
        <button class="prompt-delete" data-id="${p.id}" style="background:transparent;border:1px solid rgba(239,68,68,0.25);color:rgba(239,68,68,0.7);padding:7px 9px;border-radius:8px;cursor:pointer;display:flex;align-items:center;transition:all 0.15s ease;">${lucide_icons_default.get("trash2", 12)}</button>
      </div>
    `;
  ...
  `;
    ...
    modal.querySelector("#prompt-save").addEventListener("click", () => {
      ...
      const category = modal.querySelector("#prompt-category").value.trim() || "Generale";
      if (!title || !content) {
        showToast("Inserisci titolo e contenuto", "warning");
        return;
      }
  ...
  `;
    ...
    profiles.forEach((p) => {
      ...
      card.addEventListener("mouseenter", () => {
        if (!isActive) {
          card.style.background = theme.surfaceHover;
          card.style.borderColor = `${effectiveColor}33`;
          card.style.borderLeftColor = effectiveColor;
        }
      });
      card.addEventListener("mouseleave", () => {
        if (!isActive) {
          card.style.background = displayBg;
          card.style.borderColor = displayBorder;
          card.style.borderLeftColor = effectiveColor;
        }
      });
      ...
      <div style="display:flex;gap:6px;">
        <button class="profile-activate" data-id="${p.id}" style="flex:1;border:none;padding:7px;border-radius:8px;cursor:pointer;font-size:11px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:5px;transition:all 0.15s ease;background:${isActive ? "rgba(255,255,255,0.06)" : effectiveColor};color:${isActive ? theme.textSecondary : "white"};border:1px solid ${isActive ? "rgba(255,255,255,0.1)" : effectiveColor};">${isActive ? `${lucide_icons_default.get("x", 11)} Disattiva` : `${lucide_icons_default.get("check", 11)} Attiva`}</button>
        <button class="profile-delete" data-id="${p.id}" style="background:transparent;border:1px solid rgba(239,68,68,0.25);color:rgba(239,68,68,0.7);padding:7px 9px;border-radius:8px;cursor:pointer;display:flex;align-items:center;transition:all 0.15s ease;">${lucide_icons_default.get("trash2", 12)}</button>
      </div>
    `;
  ...
  `;
    ...
    modal.querySelector("#profile-save").addEventListener("click", () => {
      ...
      const instructions = modal.querySelector("#profile-instructions").value.trim();
      if (!name || !instructions) {
        showToast("Inserisci nome e istruzioni", "warning");
        return;
      }
  ...
  function startGuidedTour() {
    ...
    `;
      ...
      const nextBtn = popup2.querySelector("#tour-next");
  ...
  `;
    ...
    const plansCardsHtml = PLANS.map((plan) => {
      ...
      const featuresListHtml = plan.features.map((feat) => {
        const icon = feat.included ? lucide_icons_default.get("check", 14) : `<span style="opacity:0.35;">${lucide_icons_default.get("x", 14)}</span>`;
  ...
  `;
    ...
    popup.querySelectorAll(".pricing-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (plan === "free") {
          showToast("Sei gi\xE0 sul piano Free.", "info");
        } else if (plan === "pro") {
  ...
  function showTeamContactForm() {
    ...
    <div style="background:${theme.surface};border:1px solid ${theme.border};border-radius:14px;padding:24px;width:380px;max-width:90vw;position:relative;">
      <button id="infolders-contact-close" style="position:absolute;top:12px;right:12px;background:none;border:none;color:${theme.textSecondary};cursor:pointer;font-size:20px;">\xD7</button>
  ...
  `;
    ...
    popup.querySelector("form").addEventListener("submit", async (e) => {
      ...
      const status = popup.querySelector("#infolders-contact-status");
      ...
      try {
        ...
        status.textContent = err.message || "Errore invio. Riprova pi\xF9 tardi.";
  ...
  var BOOKMARK_CHATGPT_SPEC = {
    anchorSelectors: ['aside a[href*="/c/"]', 'nav a[href*="/c/"]'],
    hrefTest: (href) => /\/c\/[a-f0-9-]{8,}/i.test(href || ""),
    inSidebar: (a) => a.closest("aside") || a.closest("nav") || a.closest('[data-testid*="sidebar"]')
  };
  var BOOKMARK_CLAUDE_SPEC = {
    ...
    hrefTest: (href) => {
      try {
        ...
        return (host === "claude.ai" || host.endsWith(".claude.ai")) && /\/chat\/[^/?#]+/.test(u.pathname);
      } catch {
        return false;
      }
    },
  ...
  function injectBookmarkButtons() {
    try {
      ...
      anchors.forEach((anchor) => {
        ...
        const bookmarkSvg = lucide_icons_default.element("bookmark", 14);
        ...
        btn.addEventListener("click", (e) => {
          ...
          const checkSvg = lucide_icons_default.element("check", 14);
          btn.appendChild(checkSvg);
          if (_bookmarkBtnTimer) clearTimeout(_bookmarkBtnTimer);
          _bookmarkBtnTimer = setTimeout(() => {
            btn.innerHTML = "";
            btn.style.color = `${theme.primary}cc`;
            const restoredSvg = lucide_icons_default.element("bookmark", 14);
  ...
  chrome.storage.onChanged.addListener((changes, ns) => {
    if (ns === "local") {
      ...
      if (changes.infolders_bookmarks) {
        bookmarks = changes.infolders_bookmarks.newValue || [];
        if (isSidebarOpen) {
          const container = document.getElementById("infolders-sidebar-content");
          if (container) renderBookmarksList(container);
        }
      ...
      if (changes.infolders_premium) {
        infoldersPremium = changes.infolders_premium.newValue || null;
      }
    }
  });
  ...
  new MutationObserver(() => {
    try {
      ...
      console.warn("InFolders observer:", e);
    }
